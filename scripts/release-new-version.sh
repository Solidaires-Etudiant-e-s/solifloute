...
REPO="Solidaires-Etudiant-e-s/solifloute"
MANIFEST="../SoliFloute_ynh/manifest.toml"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"

cd "$PROJECT_DIR"

echo "=== Solifloute Release Script ==="
echo
...
YNH_DIR="$PROJECT_DIR/../SoliFloute_ynh"

echo "Committing changes in SoliFloute_ynh..."
cd "$YNH_DIR"
git add manifest.toml
git commit -m "Upgrade solifloute to v$NEXT_VERSION"
git push
